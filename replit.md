...
### Защита Root-Пользователя от Самоблокировки (23.01.2025)
- Реализована критическая защита безопасности: root-пользователь (ID=1) не может быть деактивирован или заблокирован
- Добавлена защита на уровне хранилища (storage.ts): методы updateUser, deactivateUser, lockUserAccount
- Создана защита на уровне API (auth-routes.ts): маршруты PUT /users/:id, POST /users/:id/deactivate
- Добавлены информативные сообщения об ошибках на русском языке при попытке заблокировать root'а
- Система автоматически предотвращает потерю доступа к админ панели из-за случайной деактивации root'а
- Логирование попыток блокировки root'а с предупреждениями в консоль для мониторинга безопасности